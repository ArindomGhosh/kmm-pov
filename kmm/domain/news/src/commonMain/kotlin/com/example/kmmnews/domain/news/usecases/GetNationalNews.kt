...
import com.example.kmmnews.core.network.NewsApiService
...
class GetNationalNews(
    ...
    suspend fun loadNationalNews(country: String): Flow<Entity<List<Article>>> {
        return NewsApiService.create().getNationalNews(country)
            .map { apiResponse ->
                when (apiResponse) {
                    is ApiResponse.Fail -> {
                        when (apiResponse.err) {
